    ...
    for element_1 in first_list:
        ...
        try:
            ...
            if menu1 != location1:
                ...
                # print u'%s' % menu1
                print u'%s-与当前位置不匹配' % colors.printRed(menu1)
                continue
        except:
            driver.switch_to.default_content()
            ja = 'window.scroll(0,0)'
            ...
            # print u'\033[31m%s \033[0m不匹配，已截图' %menu1
            # print u'%s' % menu1
            print u'%s-页面报错' % colors.printRed(menu1)
            continue
        # 第二层判断是否有添加按钮，打开是否报错
        try:
            frame_value = driver.find_element_by_id('mainFrame')
        ...
        try:
            ...
            if text_1 != location1:
                ...
                # print u'\033[31m%s \033[0m不匹配，已截图' %menu1
                # print u'%s' % menu1
                print u'%s-新增页面不匹配' % colors.printRed(menu1)
        except:
            ja = 'window.scroll(0,0)'
            driver.execute_script(ja)
            driver.get_screenshot_as_file(error_file1)
            # print u'\033[31m%s \033[0m不匹配，已截图' %menu1
            # print u'%s' % menu1
            print u'%s-新增页面报错' % colors.printRed(menu1)
    # 获取第二栏及后面菜单
            ...
            if menu2 == u'课程分类' or menu2 == u'公司档案' or menu2 == u'职位管理' or menu2 == u'部门':
                ...
                try:
                    ...
                    if menu2 != location2:
                        ...
                        # print u'%s' % menu2
                        print u'%s-与当前位置不匹配' % colors.printRed(menu2)
                except:
                    driver.switch_to.default_content()
                    ja = 'window.scroll(0,0)'
                    driver.execute_script(ja)
                    ...
                    # print u'\033[31m%s \033[0m不匹配，已截图' %menu2
                    # print u'%s' % menu2
                    print u'%s-页面报错' % colors.printRed(menu2)
            elif menu2 == u'课程管理':
                try:
                    element_3.click()
                except:
                ...
                try:
                    ...
                    if text_2 != location2:
                        ...
                        # print u'\033[31m%s \033[0m不匹配，已截图' %menu2
                        # print u'%s' % menu2
                        print u'%s-新增页面不匹配' % colors.printRed(menu2)
                except:
                    driver.switch_to.default_content()
                    ja = 'window.scroll(0,0)'
                    driver.execute_script(ja)
                    ...
                    # print u'\033[31m%s \033[0m不匹配，已截图' %menu2
                    # print u'%s' % menu2
                    print u'%s-新增页面报错' % colors.printRed(menu2)
            else:
                try:
                    element_3.click()
                except:
...
file_1.close()
print u'总共用时:{:.0f}秒'.format(times)
